...
video = cv2.VideoCapture("nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)1280, height=(int)720,format=(string)NV12, framerate=(fraction)24/1 ! nvvidconv flip-method=0 ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink")
...
while True:
    ...
    if not detected:
        ...
        if np.sum(num_detects) > M: #duty cycle from M of N
            ...
            #tracker = cv2.tracking.TrackerMedianFlow_create()
        ...
        bbox = convert_to_tracking_format(boxes,0)
            #ok = tracker.init(frame, bbox)
        path_mask[cY - 40:cY + 40, cX - 40:cX + 40] = 255
